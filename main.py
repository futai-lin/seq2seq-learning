...
from data_preprocess_2 import *
from util import * 


tf.app.flags.DEFINE_integer("hid_size", 500, "Size of each layer.")
tf.app.flags.DEFINE_integer("emb_size", 400, "Size of embedding.")
tf.app.flags.DEFINE_integer("fe_size", 50, "Size of embedding.")
...
tf.app.flags.DEFINE_integer("fe_vocab", 1480,'vocabulary size')
tf.app.flags.DEFINE_integer("pos_vocab", 31,'vocabulary size')
...
tf.app.flags.DEFINE_integer("limits", 200000,'max data set size')


tf.app.flags.DEFINE_boolean("hybrid_attention", True,'hybrid attention layer or normal attention')
tf.app.flags.DEFINE_boolean("fgate_encoder", True,'add field gate in encoder lstm')

tf.app.flags.DEFINE_boolean("feature", False,'concat field information to word embedding')
...
tf.app.flags.DEFINE_boolean("decoder_pos", True,'position information in hybrid attention decoder')
...
def evaluate(sess, dataloader, model, ksave_dir, mode='valid'):
    if mode == 'valid':
        ...
        texts_path = "processed_data/valid/valid_table_val"
        gold_path = gold_path_valid
        evalset = dataloader.dev_set
    else:
        # texts_path = "original_data/test.summary"
        texts_path = "processed_data/test/test_table_val"
...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        model = Seq2Seq(batch_size=FLAGS.batch_size, hid_size=FLAGS.hid_size, emb_size=FLAGS.emb_size,
                        fe_size=FLAGS.fe_size, pos_size=FLAGS.pos_size, fe_vocab=FLAGS.fe_vocab,
                        source_vocab=FLAGS.source_vocab, pos_vocab=FLAGS.pos_vocab,
                        target_vocab=FLAGS.target_vocab, scope_name="seq2seq", name="seq2seq",
                        fe_concat=FLAGS.feature, pos_concat=FLAGS.position,
                        fgate_en=FLAGS.fgate_encoder, hybrid_att=FLAGS.hybrid_attention, de_add_pos=FLAGS.decoder_pos,
                        en_add_pos=FLAGS.encoder_pos, learning_rate=FLAGS.learning_rate)
